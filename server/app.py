...
from Features.WisCoreDemo import WisCoreDemoStore
...
def _auth(auth: HTTPAuthorizationCredentials = Depends(security)):
    ...
    try:
        ...
        # normalize client/tenant claim; default to 'elio' for existing tokens
        payload.setdefault('client', 'elio')
        ...
        identity.setdefault('client', 'elio')
        identity["is_admin"] = _is_admin(identity)
        return identity


def _require_client(client_name: str):
    def _checker(identity: dict = Depends(_auth)):
        # Superuser bypass from env
        super_user = os.getenv('ELIO_SUPERUSER_EMAIL', '').lower()
        user = (identity.get('user') or '').lower()
        if user == super_user or _is_admin(identity):
            return identity
        if identity.get('client') != client_name:
            raise HTTPException(status_code=403, detail=f"Tenant '{client_name}' access required.")
        return identity
    return _checker
...
class WisCoreAssistantIn(BaseModel):
    message: str = Field(min_length=1, max_length=2000)


class WisCoreRfqIn(BaseModel):
    name: str = Field(min_length=1, max_length=120)
    company: str = Field(min_length=1, max_length=160)
    email: str = Field(min_length=3, max_length=180)
    phone: str = Field(default="", max_length=80)
    territory: str = Field(default="other", max_length=80)
    language: str = Field(default="en", max_length=12)
    category: str = Field(default="other", max_length=80)
    description: str = Field(min_length=1, max_length=4000)
    urgency: str = Field(default="standard", max_length=40)


class WisCoreLeadUpdateIn(BaseModel):
...
def login(payload: LoginIn):
    ...
    if user_record and verify_password(payload.password, user_record["password"]):
        ...
        # Determine requested client/tenant and enforce restrictions
        requested_client = (payload.client or "elio").strip().lower()
        super_user = os.getenv('ELIO_SUPERUSER_EMAIL', '').lower()
        user_email = (user_record.get("email") or "").lower()
        if requested_client != "elio":
            # Only allow switching to another client if user is admin or superuser
            if not user_record.get("is_admin") and user_email != super_user:
                return JSONResponse(status_code=403, content={"ok": False, "detail": "Insufficient privileges for requested tenant."})
...
def _wiscore_store():
    return WisCoreDemoStore()


# Rate limiter configuration (Redis-backed if ELIO_REDIS_URL is provided, else in-memory)
try:
    import redis as _redis  # optional dependency
except Exception:
    _redis = None

REDIS_URL = os.getenv('ELIO_REDIS_URL', '').strip() or None
redis_client = None
...
_assistant_public_rate = {}
ASSISTANT_PUBLIC_WINDOW = int(os.getenv('WISCORE_PUBLIC_WINDOW_SECONDS', '60'))
ASSISTANT_PUBLIC_MAX = int(os.getenv('WISCORE_PUBLIC_MAX_REQUESTS', '5'))


def _assistant_check_rate(ip: str) -> bool:
    """Return True if request allowed, False if rate limit exceeded."""
    now = int(time.time())
    if redis_client:
        key = f"wl:assistant:{ip}"
        try:
            # INCR and set expiry if first
            count = redis_client.incr(key)
            if int(count) == 1:
                redis_client.expire(key, ASSISTANT_PUBLIC_WINDOW)
            return int(count) <= ASSISTANT_PUBLIC_MAX
        except Exception:
            # fallback to in-memory on redis error
            pass

    # in-memory fallback (per-process)
    bucket = _assistant_public_rate.setdefault(ip, [])
    cutoff = time.time() - ASSISTANT_PUBLIC_WINDOW
    while bucket and bucket[0] < cutoff:
        bucket.pop(0)
    if len(bucket) >= ASSISTANT_PUBLIC_MAX:
        return False
    bucket.append(time.time())
    return True


@app.get("/wiscore/state")
def wiscore_state(identity: dict = Depends(_require_client('wiscore'))):
    return _wiscore_store().get_state()


@app.post("/wiscore/reset")
def wiscore_reset(identity: dict = Depends(_require_client('wiscore'))):
    return _wiscore_store().reset()


@app.post("/wiscore/assistant")
def wiscore_assistant(payload: WisCoreAssistantIn, identity: dict = Depends(_require_client('wiscore'))):
    return _wiscore_store().source_answer(payload.message)


@app.post("/wiscore/assistant_public")
def wiscore_assistant_public(payload: WisCoreAssistantIn, request: Request):
    # Rate-limit by client IP (Redis-backed when configured)
    ip = (request.client.host if request.client else 'unknown')
    allowed = _assistant_check_rate(ip)
    if not allowed:
        raise HTTPException(status_code=429, detail="Rate limit exceeded for public assistant")
    # Use the same answer generator but keep it lightweight on the demo store
    return _wiscore_store().source_answer(payload.message)


@app.post("/wiscore/rfq")
def wiscore_rfq(payload: WisCoreRfqIn):
    return _wiscore_store().submit_rfq(payload.dict())


@app.post("/wiscore/emergency")
def wiscore_emergency(payload: WisCoreRfqIn):
    return _wiscore_store().submit_emergency(payload.dict())


@app.patch("/wiscore/leads/{lead_id}")
def wiscore_update_lead(lead_id: str, payload: WisCoreLeadUpdateIn, identity: dict = Depends(_require_client('wiscore'))):
    try:
        return _wiscore_store().update_lead(lead_id, status=payload.status, owner=payload.owner)
    except KeyError as exc:
        raise HTTPException(status_code=404, detail="Lead not found") from exc


@app.get("/wiscore/leads/{lead_id}/quote-draft")
def wiscore_quote_draft(lead_id: str, identity: dict = Depends(_require_client('wiscore'))):
    try:
        return _wiscore_store().quote_draft(lead_id)
    except KeyError as exc:
        raise HTTPException(status_code=404, detail="Lead not found") from exc
...
# NOTE: Static files mount moved to end of file so API routes are registered first.
...
def api_info(request: Request):
    """Basic API descriptor and example curl commands for local testing."""
    origin = str(request.base_url).rstrip("/")
    return {
        "ok": True,
        "description": "Local API helper: sample endpoints and curl examples.",
        "examples": {
            "health": f"curl -sS {origin}/health",
            "rfq": f"curl -sS -X POST {origin}/wiscore/rfq -H 'Content-Type: application/json' -d '{{\"name\":\"Alice\",\"company\":\"ACME\",\"email\":\"a@b.com\",\"description\":\"Need 10\",\"items\":[{{\"sku\":\"X\",\"qty\":1}}]}}'",
            "emergency": f"curl -sS -X POST {origin}/wiscore/emergency -H 'Content-Type: application/json' -d '{{\"name\":\"Bob\",\"company\":\"ACME\",\"email\":\"b@c.com\",\"description\":\"Safety\",\"message\":\"Help\"}}'",
            "assistant_public": f"curl -sS -X POST {origin}/wiscore/assistant_public -H 'Content-Type: application/json' -d '{{\"message\":\"Find me cable trays\"}}'",
            "login": f"curl -sS -X POST {origin}/login -H 'Content-Type: application/json' -d '{{\"username\":\"<user>\",\"password\":\"<pw>\",\"client\":\"wiscore\"}}'",
        },
    }


@app.get("/wiscore/debug/state")
def wiscore_debug_state(request: Request):
    """Return WisCore state for local debugging only (restricted to localhost unless ELIO_ALLOW_DEBUG=1)."""
    allow = os.getenv("ELIO_ALLOW_DEBUG", "false").strip().lower() in {"1", "true", "yes", "on"}
    if not allow and not _is_local_request(request):
        raise HTTPException(status_code=403, detail="Debug endpoint only available from localhost or with ELIO_ALLOW_DEBUG")
    return _wiscore_store().get_state()
